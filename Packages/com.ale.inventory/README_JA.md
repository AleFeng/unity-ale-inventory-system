...
ウィンドウは上から順に、**ヘッダー**（タイトル / バージョン / 言語切り替えボタン）と 5 つの領域で構成されます。

### 多言語（エディタ UI の言語切り替え）

ヘッダーのサブタイトル下に「**中文 / English / 日本語**」の 3 ボタンが中央揃えで並び、現在の言語がハイライトされます。クリックすると、**このウェルカムウィンドウと `Inventory Editor` 設定エディタ**（6 つのサブシステムタブの全パネルと設定ドロワーを含む）の UI 言語が切り替わります。選択は `EditorPrefs` で永続化され、Unity セッションをまたいで保持されます。両方のウィンドウを開いている場合は同時に更新されます。

> これは**エディタ UI の文言のみ**に影響し、ランタイムのコンテンツローカライズ（`IS_LOCALIZATION` / Unity Localization）とは無関係です。

### 多言語設定

| 項目 | 既定 | 説明 |
|------|------|------|
| 列挙値 | 未チェック | チェックすると、属性フィールド型（`EFieldType`）、ショップ種類、更新周期、更新時間タイプなど**列挙ドロップダウンの表示名**も言語に合わせて切り替わります。未チェックの場合は常にコード内の英語の列挙識別子を表示します。 |
...
- **Unity Localization** の切り替え下では「ローカライズフォント」を設定でき、ウィザードが Prefab を生成する際に `LocalizedFontEvent` に割り当てられます（同時に `IS_TMP` の有効化が必要）。
