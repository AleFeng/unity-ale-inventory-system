...
| Addressable ツールウィンドウを開く | （`ATK_ADDRESSABLE` 有効時）アセット参照 Object ↔ AssetReference(GUID) の一括相互変換 |
| ローカライズツールウィンドウを開く | （`ATK_LOCALIZATION` 有効時）多言語テーブルの生成 / 関連付け、すべての Text フィールドのキーをワンクリック生成 |
