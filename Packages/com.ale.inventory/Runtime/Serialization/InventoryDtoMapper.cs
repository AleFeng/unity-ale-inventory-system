...
namespace Ale.Inventory.Runtime.Serialization
{
    ...
    /// 以及各分部共用的辅助方法。其余系统见 <c>InventoryDtoMapper.Inventory.cs</c> /
    /// <c>InventoryDtoMapper.Shop.cs</c>。</para>
    /// </summary>
    public static partial class InventoryDtoMapper
    {
        ...
        ///   <item>v6：导出补齐——新增 仓库 / 整理选项 / 数字格式 / 商店 各列表，
        ///         并补上道具系统此前静默丢弃的字段（模板色点、weight / stackLimit / hideInInventory、
        ///         功能标签的 UI 显示配置）。</item>
        /// </list>
        /// </summary>
        public const int Version = 6;

        /// <summary>首个包含仓库 / 商店等扩展数据块的格式版本（二进制读取按此做向后兼容判断）。</summary>
        ...
        private static FunctionTagDto ToDto(FunctionTag t, IAssetRefResolver resolver)
        {
            return new FunctionTagDto
            {
                ...
                // 有实时引用 → 经解析器转 GUID；无实时引用（Addressable 授权）→ 直接用授权地址。
                backgroundSpriteGuid = t.backgroundSprite != null
                    ? resolver.ToGuid(t.backgroundSprite)
                    : t.backgroundSpriteAddress,
