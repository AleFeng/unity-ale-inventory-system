...
namespace Ale.Inventory.Runtime
{
    ...
    public class EquipmentRuntimeManager : InventorySystemSingleton<EquipmentRuntimeManager>
    {
        ...
        /// <summary>
        /// 计算 装备组的 总属性加成：解析装备组、取出该组已装备槽位，转调
        /// <see cref="EquipmentBonusCalculator.Compute"/>（计算逻辑见该类，与实例状态解耦）。
        /// 顺序与 <see cref="EquipmentGroup.attributeDisplays"/> 一致；UI 可按
        /// <see cref="EquipmentBonus.GroupTag"/> 分组显示。
        /// </summary>
        public List<EquipmentBonus> GetTotalBonuses(string groupId)
        {
            var group = ResolveGroup(groupId);
            if (group == null) return new List<EquipmentBonus>();
            _equipped.TryGetValue(groupId, out var slots);
            return EquipmentBonusCalculator.Compute(group, slots);
        }
