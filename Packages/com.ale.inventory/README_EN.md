...
- **Item right-click menu** (`1.14.0`): right-clicking any item cell (grid / detail row / equipment candidate) opens a
  ...
  then `TryRemoveItem` on that exact slot). When the item is flagged **Cannot Discard** (`Item.noDiscard`) the Discard entry
  stays visible but greyed out (the menu can switch to hiding it) and the discard popup will not open — for key story / quest
  items; removals driven by gameplay code (selling, crafting consumption, equipment replacement) ignore the flag. Upper systems contribute entries through
