...
namespace Ale.Inventory.Editor
{
    ...
    /// </summary>
    public class ShopTemplatePanel
    {
        private ReorderableList         _masterList;
        private List<ShopTemplate>      _boundMasterList;
        ...
        public int DrawMasterList(IInventoryEditorContext ctx, int selectedIndex)
        {
            ...
            EditorGUILayout.LabelField("商店模板", InventoryEditorStyles.Header);
            if (GUILayout.Button("+", GUILayout.Width(24)))
            {
                ctx.RecordUndo("添加商店模板");
                list.Add(new ShopTemplate("新商店模板"));
                ctx.MarkDirty();
        ...
        private void BuildMasterList(List<ShopTemplate> list)
        {
            _boundMasterList = list;
            _masterList = new ReorderableList(list, typeof(ShopTemplate),
            ...
            {
                _masterCtx.RecordUndo("调整商店模板顺序");
