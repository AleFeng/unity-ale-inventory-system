...
namespace Ale.Inventory.Editor
{
    ...
    /// 分组标签用于对蓝图分组（蓝图选 1 主分组 + 多副分组），仅承载基础信息。仿 <see cref="FunctionTagPanel"/>。
    /// </summary>
    public class CraftingGroupTagPanel
    {
        private ReorderableList         _masterList;
        private List<CraftingGroupTag>  _boundList;
        ...
        public int DrawMasterList(IInventoryEditorContext ctx, int selectedIndex)
        {
            ...
            if (GUILayout.Button("+", GUILayout.Width(24)))
            {
                ...
                list.Add(new CraftingGroupTag(GenerateId(db), "新分组"));
                ctx.MarkDirty();
        ...
        private void BuildMasterList(List<CraftingGroupTag> list)
        {
            _boundList  = list;
            _masterList = new ReorderableList(list, typeof(CraftingGroupTag),
        ...
        public void DrawInspector(IInventoryEditorContext ctx, CraftingGroupTag tag)
        {
            if (tag == null)
            {
        ...
        {
            int n = db.CraftingGroupTags.Count + 1;
            string id;
            do { id = "group_" + n; n++; }
            while (db.GetCraftingGroupTag(id) != null);
            return id;
        }
    }
}
