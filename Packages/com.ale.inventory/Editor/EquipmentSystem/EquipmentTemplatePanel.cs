...
namespace InventorySystem.Editor
{
    ...
    /// </summary>
    public class EquipmentTemplatePanel
    {
        private ReorderableList               _masterList;
        private List<EquipmentGroupTemplate>  _boundMasterList;
        ...
        {
            var db   = ctx.Database;
            var list = db.EquipmentGroupTemplates;
            _masterCtx = ctx;
            ...
            EditorGUILayout.LabelField("装备组模板", InventoryEditorStyles.Header);
            if (GUILayout.Button("+", GUILayout.Width(24)))
            {
                ctx.RecordUndo("添加装备组模板");
                list.Add(new EquipmentGroupTemplate("新装备组模板"));
                ctx.MarkDirty();
        ...
        private void BuildMasterList(List<EquipmentGroupTemplate> list)
        {
            _boundMasterList = list;
            _masterList = new ReorderableList(list, typeof(EquipmentGroupTemplate),
            ...
            {
                _masterCtx.RecordUndo("调整装备组模板顺序");
        ...
        public void DrawInspector(IInventoryEditorContext ctx, EquipmentGroupTemplate template)
        {
            if (template == null)
            {
