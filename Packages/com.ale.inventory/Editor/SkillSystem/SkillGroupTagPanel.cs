...
namespace InventorySystem.Editor
{
    ...
    /// 分组标签用于对技能分组（技能选 1 主分组 + 多副分组），仅承载基础信息。仿 <see cref="CraftingGroupTagPanel"/>。
    /// </summary>
    public class SkillGroupTagPanel
    {
        private ReorderableList         _masterList;
        private List<SkillGroupTag>     _boundList;
        ...
        public int DrawMasterList(IInventoryEditorContext ctx, int selectedIndex)
        {
            ...
            if (GUILayout.Button("+", GUILayout.Width(24)))
            {
                ...
                list.Add(new SkillGroupTag(GenerateId(db), "新分组"));
                ctx.MarkDirty();
        ...
        private void BuildMasterList(List<SkillGroupTag> list)
        {
            _boundList  = list;
            _masterList = new ReorderableList(list, typeof(SkillGroupTag),
        ...
        public void DrawInspector(IInventoryEditorContext ctx, SkillGroupTag tag)
        {
            if (tag == null)
            {
        ...
        {
            int n = db.SkillGroupTags.Count + 1;
            string id;
            do { id = "skill_group_" + n; n++; }
            while (db.GetSkillGroupTag(id) != null);
            return id;
        }
    }
}
