...
- [ショップシステム（Shop System）](#ショップシステムshop-system)
- [📜 目次](#-目次)
- [コアコンセプト](#コアコンセプト)
- [ショップ種類](#ショップ種類)
- [タブ構成](#タブ構成)
...
> **進捗は安定した `guid` で特定されます（1.5.0）**：各取引進捗は「商品グループ `guid` + 商品 `guid`」で
> 記録されます。どちらも作成時に割り当てられ以後変化しないため、**グループ名の変更や、商品・商品グループの
> ドラッグ並べ替えでも既存のセーブがずれません**。1.4.0 以前のデータにはこのフィールドがないため、
> 設定エディタでデータベースを開くと自動的に補完されます（**アセットの保存を忘れずに**）。
> `guid` のないエントリは旧キー（`グループ名` + `グループ内インデックス:アイテムID`）にフォールバックし、
> 挙動は 1.4.0 と完全に同じです。旧キーで書かれたセーブは、その商品の進捗を初めて照会したときに
> **その場で移行**されるため、蓄積された取引回数は失われません。
