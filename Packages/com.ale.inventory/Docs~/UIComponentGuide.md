...
在 Package Manager 里选中本包 → `Samples` → 导入 **Inventory System Demo**，打开其中的示例场景直接进 Play。

样本内已包含成套资产：数据库 `InventoryDatabase`、效果库 `EffectDatabase`、全部 UI 预制体
（道具格子 / 各列表 / 页签 / 悬停弹窗 / 右键菜单与三个操作弹窗 / 背包 · 商店 · 制作 · 装备面板）
以及管理器预制体 `InventoryManager`（已接好数据库与各覆盖式 UI 预制体）。

> **样本即事实来源**：早期版本靠欢迎窗口的「预制体生成」向导现场生成这些资产，该向导已移除
> —— 生成器与手工调整过的预制体长期双轨很容易互相覆盖。现在直接以样本内的预制体与配置数据为准，
> 需要改就直接改样本资产。
