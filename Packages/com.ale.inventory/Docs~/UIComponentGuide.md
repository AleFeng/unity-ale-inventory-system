...
> **重新生成是安全的（1.6.0 起）**：预制体与数据库都是**就地覆盖**，资产 GUID 保持不变——单独重生成某个被依赖的
> 预制体（如 `PF_UiwInventoryItemCell`）或数据库，依赖它的预制体 / 管理器的引用不会断。此前是「先删除再创建」，
> GUID 会变，引用被静默打断（依赖对话框只向下遍历依赖，不会提示）。
>
