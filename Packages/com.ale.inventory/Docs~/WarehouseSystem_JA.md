...
| 自動整理 | チェックするとアイテム変化のたびにソートルールで自動整理。**昇順 / 降順の由来**：画面にソートツールバー（`UiwSortToolbar`）がある場合はその現在の方向を全ルールに適用。ツールバーが無い場合は各ソート条件が自身の昇順 / 降順設定を使います（1.6.0 で修正 —— 以前はツールバーが無いと一律降順でした） |
...
// ニューゲーム：すべての倉庫をクリアして初期の空状態に再構築
InventoryRuntimeManager.Instance.ResetAll();
```

`RuntimeInventoryState` = `inventoryId` + `slots`（順序付きスロットリスト）。

> **`LoadSaveData` はマージではなく上書き（1.6.0 より）**：まずメモリ上の状態をクリアし、データベースに従って
> 全倉庫の空の骨格を再構築（固定容量の倉庫は事前確保の空スロットを復元）してから、セーブのスロットを重ねます。
> したがって —— **データベースにあってセーブに無い**倉庫は初期の空状態に戻ります（前回の内容は残りません）。
> **セーブにあってデータベースに無い**倉庫 ID もメモリに読み込まれます（定義が見つからず、容量無制限として扱われます）——
> 破棄されるわけではありません。契約は他の 3 つのランタイムマネージャと同一です。
> [アーキテクチャ - セーブ契約](Architecture_JA.md#サブシステムのランタイムマネージャー) を参照。
