...
| 类 | 职责 |
|----|------|
| `AttributeFieldDrawer` | 按 `EFieldType` 绘制单个 `AttributeValue`。GUILayout 与 Rect 两条路径**共用同一套按类型分派的实现**（1.6.0 前为两个并行维护的大 switch） |
...
| `EditorEntityListPanel<TEntity,TTemplate>` | **中栏实体列表的泛型基类**：模板过滤页签 + 搜索栏 +「从模板添加」/「快速添加」+ 双行条目列表（拖拽句柄 / 模板色点 / 若干列 / 删除按钮）+ 拖拽重排 + 延迟删除 + 上下键导航。六个系统的中栏列表均由此派生，各自只实现 `DrawRowColumns`（列布局）与新增 / 搜索规则 |
| `EquipmentConfigDrawer` | 装备「槽位列表 + 装备属性字段」的共享绘制（装备组 Inspector 与装备组模板 Inspector 复用） |
...
`InventoryRuntimeManager` 按职责拆为多个分部文件：核心（字段 / 初始化 / 数据获取 / 道具管理 / 整理入口 / 存档）、
`.Time`（时间获取器）、`.UiHost`（覆盖式 UI 与悬停弹窗宿主）、`.TestSeed`（测试道具填充，仅编辑器 / 开发版构建）。
与实例状态无关的排序实现已独立为 `InventorySortService`。
...
- 提供 `GetSaveData / LoadSaveData / ResetAll` 接口与游戏存档系统对接（契约见下方「存档契约」）；
...
- `EquipmentRuntimeManager`：按 `装备组 ID → (槽位 ID → 已装备道具 ID)` 维护已装备状态；装备 / 卸下 / 交换与 `InventoryRuntimeManager` 协作搬运道具（放不回旧道具时回滚），限制匹配 **全部 AND**，自动找槽，按「装备属性字段列表」跨已装备道具求和得总加成（汇总算法在 `EquipmentBonusCalculator`，与实例状态解耦、可单独调用）；**有已装备状态存档**（`GetSaveData` / `LoadSaveData` / `ResetAll`）；事件 `OnEquipmentChanged`。
