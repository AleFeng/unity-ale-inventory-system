...
> **再生成は安全です（1.6.0 より）**：プレハブもデータベースも**その場で上書き**され、アセット GUID は変わりません ——
> 依存されているプレハブ（`PF_UiwInventoryItemCell` など）やデータベースを単独で再生成しても、それに依存する
> プレハブ / マネージャーの参照は切れません。以前は「削除してから作成」だったため GUID が変わり、参照が黙って
> 壊れていました（依存ダイアログは依存を下方向にしか辿らないため警告も出ませんでした）。
>
