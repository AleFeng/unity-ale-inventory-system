...
| `scrollDirection` | グリッド | `縦` | `縦`（交差軸=列、ビューポート幅で列数）/ `横`（交差軸=行、ビューポート高で行数）。Inspector には `[InspectorName]` により中国語ラベル 纵向 / 横向 が表示されます。コード上のメンバーは `EListScrollDirection.Vertical` / `.Horizontal`（1.5.0 で中国語識別子から改名。シリアライズ値は不変） |
...
Package Manager で本パッケージを選択 → `Samples` → **Inventory System Demo** をインポートし、含まれている
サンプルシーンを開いて Play してください。

サンプルには一式のアセットが含まれます：データベース `InventoryDatabase`、エフェクトライブラリ
`EffectDatabase`、全 UI プレハブ（アイテムセル / 各リスト / タブ / ホバーポップアップ / 右クリックメニューと
3 つの操作ポップアップ / バックパック・ショップ・クラフト・装備パネル）、およびデータベースと各カバー UI
プレハブを接続済みのマネージャープレハブ `InventoryManager`。

> **サンプルが正となります**：以前のバージョンではウェルカムウィンドウの「プレハブ生成」ウィザードでこれらを
> その場で生成していましたが、当該ウィザードは削除しました —— ジェネレータと手作業で調整したプレハブを併存
> させると互いに上書きし合うためです。今後はサンプル内のアセットを直接編集してください。

---

## 10.6 再利用可能な構成部品（UI 拡張時はこれらを優先）

以下はいずれも**純粋な C# のヘルパー / 基底クラス**で、GameObject にアタッチするコンポーネントではありません。
独自 UI の構築や本プラグインの画面を拡張する際は、同じ骨組みを書き直さずこれらを再利用してください。
...
**`UiwTabStrip` —— 差分再利用**：`SetTabs` は無条件に破棄・再生成しません。個数が変わらないときは既存インスタンスを
...
{
    var w = _pool.Next();
    if (!w) break;              // プレハブ / コンテナ未設定
    w.SetData(d);
}
_pool.End();                    // 余剰インスタンスは SetActive(false)
```

生成時の一度きりの初期化（イベント購読など）は `Next(out bool created)` を使います —— 再利用されたインスタンスは
二重購読しないため、「先に外してから足す」という重複防止の書き方は不要です。回収動作の既定は `SetActive(false)` で、
別の処理（Addressable ハンドルの解放など）が必要なら `End(Action<T>)` にデリゲートを渡します。

**`UiwHoverTooltipSource` —— なぜ `OnDisable` 経路が必要か**：オブジェクトが非アクティブになるとき（リストの回収、
パネルを閉じる、クイック装備後にそのセルが隠れる）、Unity は `OnPointerExit` を**発行しません**。そのままではポップアップが
画面に residue として残ります。そこで非アクティブ時、そのセルがポップアップを表示中なら自ら閉じます —— しかも自分が
開いた分だけで、他のセルには影響しません。**`OnPointerEnter` / `OnPointerExit` / `OnDisable` をオーバーライドする際は
必ず `base` を呼んでください**。さもないとポップアップ機能を失うか、ポップアップが残ります。
