...
> 2 つの格納形式はディスク上のフォーマットが異なり、同名フィールドで自動共有はできません。マクロ切り替え後、汎用メニュー **Tools > Ale Toolkit > Addressable**（`InventoryDatabase` をドラッグ）で、その全リソース属性値（アイコン / 背景を含む）を「Object 参照 ↔ AssetReference(GUID)」間でワンクリック相互変換できます。
>
