...
DTO 層（`Runtime/Serialization/InventoryDtoModels.cs`）はデータモデルと 1 対 1 でミラーする平坦な構造で、唯一の違いはオブジェクト参照を GUID 文字列で保持する点です。
...
5. （任意）`InventoryDtoModels.cs` に DTO ミラーを追加 —— ショップ / クラフト / 装備はいずれも行っていません。ランタイムが登録済みの `InventoryDatabase`（ScriptableObject）を直接消費するためで、DTO エクスポートは現在アイテムシステムのデータのみをカバーします。
